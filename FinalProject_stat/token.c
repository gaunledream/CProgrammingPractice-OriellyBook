...
static int in_comment = FALSE; //tru if we are in a comment
/*********************************************************
...
enum TOKEN_TYPE next_token(void){
		...
		case C_SLASH:
			...
			case C_DOUBLE:
				while (1){
					...
					if (in_cur_char() == '"')
						break;
					//escape character and then skip the next char
					if (in_cur_char() == '\\')
						in_read_char();
				}
				in_read_char();
			...
			case C_SINGLE:
				while(1){
					...
					if (in_cur_char() == '\'')
						break;
					//escape char and then skip the next char
					if (in_cur_char() == '\\')
						in_read_char();
				}
				in_read_char();
	...
	abort();
	return (T_EOF); //should never get here either, but we put
	//in the return to avoid a compiler warning.
}